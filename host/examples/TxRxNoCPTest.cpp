 ...
 **********************************************************************/
void server_work(size_t &index, bool& sync_flag) {
    // 创建 Socket
    ...
    while (not stop_signal_called) {
        ...
        if (abs(receivedData + 256) < 50 && receivedData != 0) {
            ...
            boost::lock_guard<boost::mutex> lock(syncFlagMutex);
            sync_flag = true;            // 同步完成，开始接收数据
    ...
    while (not stop_signal_called
        ...
        if (sync_flag) {
            for (size_t i = 0; i < outfiles.size(); i++) {
                outfiles[i]->write(
                    (const char*)buff_ptrs[i], num_rx_samps / 100 * sizeof(samp_type)); // 降采样100倍
            }
        }
    }
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    boost::thread_group server_Group;
    // Start the thread
    server_Group.create_thread(boost::bind(&server_work, boost::ref(index), boost::ref(sync_flag)));
